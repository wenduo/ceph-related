...
def rados_write(ioctx, key, lenth):
    ...
    time1 = time.time()
    r=ioctx.write(key, value)
    time2 = time.time()
    write_time = time2-time1
    return r,write_time
def rados_write_full(ioctx, key, lenth):
    value = random_str(1)*lenth    
    time1 = time.time()
    ...
    write_time = time2-time1
    return r,write_time
def rados_aio_write(ioctx, key, lenth):
    value = random_str(1)*lenth    
    time1 = time.time()
    r=ioctx.aio_write_full(key, value)
    time2 = time.time()
    write_time = time2-time1
    return r,write_time
def rados_aio_write_full(ioctx, key, lenth):
    value = random_str(1)*lenth    
    time1 = time.time()
    r=ioctx.write_full(key, value)
    time2 = time.time()
...
if __name__ == "__main__":
    ...
    data_lenths = [500000];
    #process_nums =[1,5,10,15,20,30,40,50,60,100];
    process_nums =[1];
    for k in data_lenths:
        for v in process_nums:
		   ...
		   processes.append( BenchProcess(i,rados_write,data_lenth,data_num,q))
	       time1 = time.time()
	       for i in processes:
		   i.start()
	       for i in processes:
